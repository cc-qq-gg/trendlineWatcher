...
from Config import *
from config_constants import DINGTALK_ROBOT_ID, DINGTALK_ROBOT_SECRET
...
def ccxt_fetch_future_account(exchange, max_try_amount=5):
    ...
    for _ in range(max_try_amount):
        try:
            balance_of = float(
                # 应用可用保证金？
                exchange.private_get_account_balance({"ccy": "USDT"})["data"][0][
                    "details"
                ][0]["availEq"]
            )
            df = pd.DataFrame(exchange.private_get_account_positions()["data"])

            # 只将数值列转换为float，非数值列保持原状
            ...
            print(
                "通过ccxt的通过futures_get_accounts获取所有合约账户信息，失败，稍后重试：\n",
                e,
            )
...
def ccxt_fetch_future_position(exchange, max_try_amount=5):
    ...
    for _ in range(max_try_amount):
        try:
            ...
            print(
                "通过ccxt的通过futures_get_position获取所有合约的持仓信息，失败，稍后重试。失败原因：\n",
                e,
            )
            time.sleep(medium_sleep_time)
...
# ===通过ccxt获取K线数据
def ccxt_fetch_candle_data(
    exchange: ccxt.okx, symbol, time_interval, limit, max_try_amount=5
):
    """
    ...
    for _ in range(max_try_amount):
        try:
            ...
            df["candle_begin_time_GMT8"] = df["candle_begin_time"].dt.tz_convert('Asia/Shanghai')
            df = df[
...
def update_symbol_info(exchange, symbol_info, symbol_config):
    ...
    if not future_position.empty:
        ...
        instrument_id_list = [
            symbol_config[x]["instrument_id"] for x in symbol_config.keys()
        ]
        future_position = future_position[
            future_position.instrument_id.isin(instrument_id_list)
        ]
        ...
        if len(symbol_info[symbol_info.duplicated("产品ID")]) > 1:
            print(
                symbol_info["产品ID"],
                "当前账户同时存在多仓和空仓，请平掉其中至少一个仓位后再运行程序，程序exit",
            )
            exit()
    return symbol_info
...
# 获取需要的K线数据，并检测质量。
def get_candle_data(
    exchange, symbol_config, time_interval, run_time, max_try_amount, candle_num, symbol
):
    """
    ...
    for i in range(max_try_amount):
        ...
        df = ccxt_fetch_candle_data(
            exchange, instrument_id, time_interval, limit=candle_num
        )
        if df.empty:
            continue  # 再次获取
        ...
        if time_interval.endswith("m") or time_interval.endswith("M"):
            _ = df[
                df["candle_begin_time_GMT8"]
                == (run_time - timedelta(minutes=int(time_interval[:-1])))
            ]
        elif time_interval.endswith("h") or time_interval.endswith("H"):
            _ = df[
                df["candle_begin_time_GMT8"]
                == (run_time - timedelta(hours=int(time_interval[:-1])))
            ]
        else:
        ...
        if _.empty:
            ...
            df = df[
                df["candle_begin_time_GMT8"] < pd.to_datetime(run_time)
            ]  # 去除run_time周期的数据
...
def single_threading_get_data(
    ...
    # 逐个获取symbol对应的K线数据
    for symbol in symbol_config.keys():
        _, symbol_candle_data[symbol], symbol_info.at[symbol, "信号价格"] = (
            get_candle_data(
                exchange,
                symbol_config,
                time_interval,
...
def calculate_signal(symbol_info, symbol_config, symbol_candle_data):
    ...
    for symbol in symbol_config.keys():
        ...
        if not df.empty:  # 当原始数据不为空的时候
            ...
            target_pos = getattr(Signals, symbol_config[symbol]["strategy_name"])(
                df, para=symbol_config[symbol]["para"]
            )
...
# 在合约市场下单
def okex_future_place_order(
    exchange, symbol_info, symbol_config, symbol_signal, max_try_amount, symbol
):
    """
    :param exchange:
    :param symbol_info:
    ...
    for order_type in symbol_signal[symbol]:
        ...
        while True:
            try:
                response = float(
                    exchange.public_get_market_ticker(
                        {"instId": symbol_config[symbol]["instrument_id"]}
                    )["data"][0]["last"]
                )
                ...
                if len(symbol_signal[symbol]) == 2 and order_type in [
                    1,
                    2,
                ]:  # 当两个条件同时满足时，说明当前处于平仓后，需要再开仓的阶段。
                    time.sleep(
                        short_sleep_time
                    )  # 短暂的休息1s，防止之平仓后，账户没有更新
                    _, symbol_info["账户余额"] = ccxt_fetch_future_account(exchange)

                # 确定下单参数
                params["side"] = "buy" if order_type in [1, 4] else "sell"
                params["px"] = str(
                    round(float(cal_order_price(response, order_type)), 2)
                )

                params["sz"] = str(
                    float(
                        cal_order_size(
                            symbol, symbol_info, symbol_config[symbol]["leverage"]
                        )
                    )
                )
                ...
                # 获取订单信息
                state = exchange.private_get_trade_order(
                ...
                if state == "live":
                    ...
                    if num >= max_try_amount:
                        send_dingding_msg(
                            "下单未成交次数超过max_try_amount，终止下单，程序不退出"
                        )
                        break
                    num += 1
                    time.sleep(2)
                ...
                # 记录失败参数，币种，信号，余额，下单参数，错误信息
                errmsg = str(e)
                content = f"""
下单失败
{symbol}:{symbol_signal[symbol]}
余额,{symbol_info.loc[symbol, "账户余额"]}
...
"""
                ...
                print(content)
                # 余额不足
                if "insufficient" in errmsg.lower():
                    break
                time.sleep(short_sleep_time)
                max_try_amount -= 1
                if max_try_amount <= 0:
                    ...
                    send_dingding_msg(
                        f"下单失败次数超过max_try_amount，终止下单，程序不退出"
                    )
...
# 串行下单
def single_threading_place_order(
    exchange, symbol_info, symbol_config, symbol_signal, max_try_amount=5
):
    """
    :param exchange:
    :param symbol_info:
    ...
    if symbol_signal:
        ...
        for symbol in symbol_signal.keys():
            # 下单
            _, order_id_list = okex_future_place_order(
                exchange,
                symbol_info,
                symbol_config,
                symbol_signal,
            ...
            for order_id in order_id_list:
                ...
                symbol_order.loc[order_id, "信号价格"] = symbol_info.loc[
                    symbol, "信号价格"
                ]
                symbol_order.loc[order_id, "信号时间"] = symbol_info.loc[
                    symbol, "信号时间"
                ]
    ...
    if symbol_order.empty is False:
        ...
        for order_id in symbol_order.index:
            ...
            for i in range(max_try_amount):
                try:
                    para = {
                        "instId": symbol_config[symbol_order.at[order_id, "symbol"]][
                            "instrument_id"
                        ],
                        "ordId": order_id,
                    }
            ...
            if order_info:
                symbol_order.at[order_id, "订单状态"] = okex_order_state[
                    order_info["data"][0]["state"]
                ]
                symbol_order.at[order_id, "开仓方向"] = okex_order_type[
                    order_info["data"][0]["posSide"]
                ]
                symbol_order.at[order_id, "委托数量"] = order_info["data"][0]["sz"]
                symbol_order.at[order_id, "成交数量"] = order_info["data"][0][
                    "accFillSz"
                ]
                symbol_order.at[order_id, "委托价格"] = order_info["data"][0]["px"]
                symbol_order.at[order_id, "成交均价"] = order_info["data"][0]["avgPx"]
                symbol_order.at[order_id, "委托时间"] = pd.to_datetime(
                    order_info["data"][0]["cTime"], unit="ms"
                )
            else:
...
# ===获取全部历史数据
def fetch_okex_symbol_history_candle_data(
    exchange, symbol, time_interval, max_len, max_try_amount=5
):
    """
    ...
    # OKX时间周期映射（支持更多时间周期）
    timeframe_mapping = {
        '1W': {'multiplier': 7 * 24 * 60 * 60 * 1000, 'desc': '1周'},
        '3D': {'multiplier': 3 * 24 * 60 * 60 * 1000, 'desc': '3天'},
        '1D': {'multiplier': 24 * 60 * 60 * 1000, 'desc': '1天'},
        '12H': {'multiplier': 12 * 60 * 60 * 1000, 'desc': '12小时'},
        '6H': {'multiplier': 6 * 60 * 60 * 1000, 'desc': '6小时'},
        '4H': {'multiplier': 4 * 60 * 60 * 1000, 'desc': '4小时'},
        '2H': {'multiplier': 2 * 60 * 60 * 1000, 'desc': '2小时'},
        '1H': {'multiplier': 1 * 60 * 60 * 1000, 'desc': '1小时'},
        '30m': {'multiplier': 30 * 60 * 1000, 'desc': '30分钟'},
        '15m': {'multiplier': 15 * 60 * 1000, 'desc': '15分钟'},
        '5m': {'multiplier': 5 * 60 * 1000, 'desc': '5分钟'},
        '3m': {'multiplier': 3 * 60 * 1000, 'desc': '3分钟'},
        '1m': {'multiplier': 1 * 60 * 1000, 'desc': '1分钟'},
    }

    # 兼容旧的时间格式（向后兼容）
    if time_interval not in timeframe_mapping:
        time_interval_int = int(time_interval[:-1])
        if time_interval.endswith("m"):
            time_segment = time_interval_int * 60 * 1000
            timeframe_mapping[time_interval] = {'multiplier': time_segment, 'desc': f"{time_interval_int}分钟"}
        elif time_interval.endswith("h"):
            time_segment = time_interval_int * 60 * 60 * 1000
            timeframe_mapping[time_interval] = {'multiplier': time_segment, 'desc': f"{time_interval_int}小时"}
        elif time_interval.endswith("d"):
            time_segment = time_interval_int * 24 * 60 * 60 * 1000
            timeframe_mapping[time_interval] = {'multiplier': time_segment, 'desc': f"{time_interval_int}天"}
        elif time_interval.endswith("w"):
            time_segment = time_interval_int * 7 * 24 * 60 * 60 * 1000
            timeframe_mapping[time_interval] = {'multiplier': time_segment, 'desc': f"{time_interval_int}周"}
        else:
            raise ValueError(f"不支持的时间周期: {time_interval}")

    # 修复时间戳计算逻辑 - 使用当前时间作为起始点
    since = now_milliseconds
    # 计算最早需要获取的时间点
    end_time = now_milliseconds - max_len * timeframe_mapping[time_interval]['multiplier']

    # 循环获取历史数据
    all_kline_data = []
    request_count = 0

    print(f"开始获取{symbol} {timeframe_mapping[time_interval]['desc']}K线数据...")
    ...
    while True:
        ...
        print(f"第{request_count}次请求: 时间点={datetime.fromtimestamp(since/1000)}")

        # 获取K线使，要多次尝试
        for i in range(max_try_amount):
            try:
                kline_data = exchange.public_get_market_candles(params=params)["data"]
                print(f"请求成功，获取到 {len(kline_data)} 条数据")
                break
            except Exception as e:
                ...
                if i == (max_try_amount - 1):
                    _ = (
                        "【获取需要交易币种的历史数据】阶段，fetch_okex_symbol_history_candle_data函数中，"
                        "使用ccxt的fetch_ohlcv获取K线数据失败，程序Raise Error"
                    )
                    send_dingding_and_raise_error(_)

        if kline_data:
            if int(kline_data[-1][0]) <= end_time:
                print(f"已达到目标时间范围，累计获取 {len(all_kline_data)} 条数据")
                break

            since = int(kline_data[-1][0])  # 更新since，为下次循环做准备
            all_kline_data += kline_data
            print(f"当前累计: {len(all_kline_data)} 条数据")

            # 防止无限循环
            if len(all_kline_data) >= max_len or request_count >= 30:
                print(f"达到获取限制或请求次数过多，结束")
                break
        # 如果没有获取到数据，尝试更少的数据量或直接返回
        if not kline_data:
            print("警告：无法获取更多历史数据，可能已达到API限制或时间范围")
            print(f"已获取 {len(all_kline_data)} 条数据，停止继续获取")
            break
    ...
    # 检查数据格式是否正确
    if len(all_kline_data) == 0 or len(all_kline_data[0]) < 6:
        print(f"错误：K线数据格式不正确，数据行数: {len(all_kline_data)}")
        return pd.DataFrame()
    ...
    try:
        ...
        # 检查DataFrame是否有足够的列
        if df.shape[1] < 6:
            print(f"错误：K线数据列数不足，期望6列，实际{df.shape[1]}列")
            return pd.DataFrame()
    ...
    print(symbol, "获取历史数据行数：", len(df), ",", datetime.now())
...
def dingding_report_every_loop(
    ...
    if symbol_signal:
        symbol_order_str = [
            "\n\n" + y.to_string() for x, y in symbol_order.iterrows()
        ]  # 持仓信息
    ...
    if run_time.minute % 30 == 0 or symbol_signal:
        send_dingding_msg(
            content, robot_id=robot_id_secret[0], secret=robot_id_secret[1]
        )


# ===为了达到成交的目的，计算实际委托价格会向上或者向下浮动一定比例默认为0.02
def cal_order_price(price, order_type, ratio=0.005):
...
def cal_order_size(symbol, symbol_info, leverage, volatility_ratio=0.98):
    ...
    size = e * l * volatility_ratio / (price * coin_value)
    size = float(f"{size:.10f}"[:4])
    return max(size, 0.01)  # 防止出现size为情形0，设置最小下单量为1
